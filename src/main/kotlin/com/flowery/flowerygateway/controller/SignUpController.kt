...
class SignUpController(
    ...
    fun findPassword(@RequestBody request: FindPasswordRequestDTO): Mono<ResponseEntity<String>> {
        ...
        return memberService.sendPasswordCode(request)
            .map { result ->
                when (result) {
    ...
    @GetMapping("/gardener/password")
    fun checkVerificationCode(@RequestBody request: EmailVerificationDto) : Mono<ResponseEntity<String>> {
        return memberService.verifyCode(request)
            .map { result ->
                when (result) {
                    "Verification successful!" -> ResponseEntity.ok().body(result)
                    "Verification failed: No code found for this email." -> ResponseEntity.status(HttpStatus.NOT_FOUND).body(result)
                    "Verification failed: Invalid code." -> ResponseEntity.status(HttpStatus.BAD_REQUEST).body(result)
                    else -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Verification failed: Server error")
                }
        }
    }
